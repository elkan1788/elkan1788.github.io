...
category: other
tags: [linux, ssh]
keywords: "SSH;Linux;密码;登录;"
description: "Linux使用SSH免密码登录"
---
...
![ssh-login-without-pswd-1]({{site.cdn.img}}/ssh-login-without-pswd-01.png{{site.cdn.img-ext}})

然后将生成的`PUB_KEY`文件, 使用`cat`管道命令输出名称为`authorized_keys`的文件, 再用`scp`命令拷贝一份到节点服务器上面(此时是要输入密码的), 如下图所示: 

![ssh-login-without-pswd-2]({{site.cdn.img}}/ssh-login-without-pswd-02.png{{site.cdn.img-ext}})
